...
#[tauri::command]
pub fn check_sim_rally_cn_install_status(rbr_install_path: String) -> Result<String, String> {
    let plugin_path = format!("{}/Plugins/SimRallyCN.dll", rbr_install_path.clone());
    let disabled_plugin_path = format!("{}.disabled", plugin_path.clone());

    if !fs::exists(plugin_path).map_err(|e| e.to_string())? {
        if fs::exists(disabled_plugin_path).map_err(|e| e.to_string())? {
            return Ok("已禁用".to_string());
        }
...
#[tauri::command]
pub async fn install_sim_rally_cn(
    ...
    let zip_file = rbr_install_path.clone() + "/sim_rally_cn_temp.zip";
